...
- Full-workspace `cargo clippy --workspace --all-targets -- -D warnings` and `pnpm.cmd verify` passed with the isolated target directory. SQLite lifecycle passed; PostgreSQL remained explicitly ignored because `SDKWORK_DATABASE_TEST_POSTGRES_URL` was not configured, and this requirement changes no database behavior.
