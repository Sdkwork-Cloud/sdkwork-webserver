...
- PostgreSQL lifecycle remains environment-gated because `SDKWORK_DATABASE_TEST_POSTGRES_URL` is not configured; SQLite lifecycle passed. This does not weaken this requirement's no-database runtime boundary, but remains a repository-level commercial release blocker.
