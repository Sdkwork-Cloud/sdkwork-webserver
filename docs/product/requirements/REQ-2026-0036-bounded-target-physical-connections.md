...
- SDKWork pagination, API operation-pattern, response-envelope, app-SDK consumer-import, application-layering, Rust backend-composition, and route-collision validators pass. SQLite lifecycle coverage passes inside workspace verification; PostgreSQL lifecycle remains ignored because `SDKWORK_DATABASE_TEST_POSTGRES_URL` is not available and is not claimed by this requirement.
