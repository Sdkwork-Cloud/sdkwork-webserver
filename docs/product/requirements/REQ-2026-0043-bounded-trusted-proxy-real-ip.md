...
- Isolated-target `pnpm.cmd verify` passes the complete workspace tests, contract tests, API materialization, formatting, repository/application standards, topology, SQLite lifecycle, and cloud gateway validation. The first default-target attempt only encountered the pre-existing Windows lock on a running gateway executable; no process was terminated. PostgreSQL remains intentionally ignored without `SDKWORK_DATABASE_TEST_POSTGRES_URL` and is not claimed.
