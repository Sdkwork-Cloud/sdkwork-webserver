...
- SDKWork pagination, API operation-pattern, response-envelope, app-SDK consumer-import, application-layering, Rust backend-composition, route-collision, and component-port validators pass. `cargo fmt --all -- --check` and `git diff --check` pass. SQLite lifecycle passes; PostgreSQL lifecycle remains ignored because no disposable `SDKWORK_DATABASE_TEST_POSTGRES_URL` is configured and is not claimed.
