...
- Full-workspace `cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --all -- --check`, and `pnpm.cmd verify` passed with the isolated target directory. SQLite lifecycle passed. PostgreSQL lifecycle remained explicitly ignored because `SDKWORK_DATABASE_TEST_POSTGRES_URL` was not configured; this requirement changes no database behavior.
