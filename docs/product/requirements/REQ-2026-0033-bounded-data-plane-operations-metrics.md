...
- Full `pnpm.cmd verify` and `cargo clippy --workspace --all-targets -- -D warnings` pass with the isolated F-drive target. SQLite lifecycle passes. PostgreSQL lifecycle remains explicitly ignored because `SDKWORK_DATABASE_TEST_POSTGRES_URL` is not configured; this requirement changes no database behavior and does not claim PostgreSQL lifecycle evidence.
