...
describe("surface SDK boundaries", () => {
  it("keeps backend SDK imports out of console packages", () => { const offenders = files(resolve(root, "packages")).filter((path) => path.includes("-console-") && readFileSync(path, "utf8").includes("@sdkwork/web-backend-sdk")); expect(offenders).toEqual([]); });
  it("keeps app SDK imports out of admin packages", () => { const offenders = files(resolve(root, "packages")).filter((path) => path.includes("-admin-") && readFileSync(path, "utf8").includes("@sdkwork/web-app-sdk")); expect(offenders).toEqual([]); });
