...
export function WebserverWorkspace({ locale, modules, onSignOut, permissionScope, registry, surface, userLabel }: WebserverWorkspaceProps) {
  ...
  return <div className="app-layout">
    ...
    <main className="workspace"><Routes><Route path="/:resource" element={<ResourcePage entry={current} locale={locale} source={registry[current.resource]} />} /><Route path="*" element={<Navigate to={`${basePath}/${current.resource}`} replace />} /></Routes></main>
  </div>;
}
...
function ResourcePage({ entry, locale, source }: { entry: { resource: WebserverResourceKey }; locale: WebserverLocale; source?: WebserverResourceDataSource }) {
  ...
  const [scopeId, setScopeId] = useState(() => sessionStorage.getItem("sdkwork.webserver.scopeId") ?? "");
  ...
  const persistScope = (value: string) => { setScopeId(value); if (value.trim()) sessionStorage.setItem("sdkwork.webserver.scopeId", value.trim()); else sessionStorage.removeItem("sdkwork.webserver.scopeId"); };
