...
export function PortalHeader({
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#07120d]/95 text-white backdrop-blur-xl">
      <div className="grid h-14 w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-2 px-4 sm:px-6 lg:px-8 xl:grid-cols-[minmax(220px,1fr)_auto_minmax(220px,1fr)] 2xl:px-10">
        <a className="flex min-w-0 items-center gap-2 justify-self-start whitespace-nowrap text-inherit no-underline" href="/" aria-label={t("brand.name")}>
          <span className="grid size-8 shrink-0 place-items-center rounded bg-emerald-400 text-emerald-950">
        ...
        <div className="flex min-w-0 items-center gap-1.5 justify-self-end whitespace-nowrap">
          <button
            ...
            className="inline-flex size-8 shrink-0 items-center justify-center rounded bg-white/[0.07] text-zinc-200 transition-colors hover:bg-white/[0.12] hover:text-emerald-300 xl:hidden"
          ...
          <a
            ...
            className="inline-flex size-8 shrink-0 items-center justify-center rounded text-zinc-300 no-underline transition-colors hover:bg-white/[0.07] hover:text-emerald-300"
          ...
          <a
            ...
            className="inline-flex size-8 shrink-0 items-center justify-center gap-2 rounded bg-emerald-400 text-[13px] font-bold text-emerald-950 no-underline transition-colors hover:bg-emerald-300 sm:w-auto sm:px-3"
          ...
          {viewer ? (
            <>
              ...
              <a
                ...
                className="flex min-w-0 items-center gap-2 whitespace-nowrap text-zinc-200 no-underline transition-colors hover:text-emerald-300"
                href={navigation.consoleHref}
                title={t("header.accountAria", { user: viewerLabel })}
              >
                <span aria-hidden="true" className="grid size-8 shrink-0 place-items-center rounded-full bg-white/10 text-xs font-bold text-emerald-200">
                  {viewerInitial}
                </span>
      ...
      {navigationOpen ? (
        ...
        >
          <div className="grid grid-cols-2 gap-x-4 px-4 py-2 sm:grid-cols-3 sm:px-6 lg:grid-cols-6 lg:px-8">
            {navigationItems.map((item) => (
              <a
                className="flex min-h-10 items-center whitespace-nowrap text-sm font-semibold text-zinc-200 no-underline transition-colors hover:text-emerald-300"
                href={item.href}
                key={item.href}
