...
export function SkillIntegrationPanel({
  ...
  return (
    <aside
      ...
      className="scroll-mt-[64px] overflow-hidden rounded bg-[#0d261b]"
      id="skill"
    >
      <header className="flex min-h-11 items-center justify-between gap-4 bg-white/[0.045] px-5">
        <span className="flex min-w-0 items-center gap-2 text-xs font-bold uppercase text-emerald-300">
          <Bot aria-hidden="true" size={16} />
          <span className="truncate">{t("skill.eyebrow")}</span>
        </span>
        <span className="flex shrink-0 items-center gap-1.5 whitespace-nowrap text-[11px] text-emerald-200">
      ...
      <div className="p-5 sm:p-6">
        ...
        <p className="mt-3 max-w-[520px] text-sm leading-6 text-emerald-50/70">{t("skill.description")}</p>
        <span className="mt-3 inline-flex items-baseline gap-1.5 text-xs text-emerald-100/60">
          <strong className="text-base text-white">{portalAgentCount}</strong>
          {t("skill.agentCountShort")}
        </span>

        <div className="mt-5 overflow-hidden rounded bg-[#06100b]">
          <div className="flex min-h-9 items-center gap-2 bg-white/[0.035] px-4 text-xs font-semibold text-emerald-100/60">
            <Terminal aria-hidden="true" size={14} />
            {t("skill.commandLabel")}
          </div>
          <pre className="m-0 min-h-28 whitespace-pre-wrap break-words p-4 font-mono text-[13px] leading-6 text-emerald-100">{instruction}</pre>
        </div>
      </div>

      <footer className="flex flex-col gap-3 bg-black/15 px-5 py-4 sm:px-6">
        <span
          className={`flex min-h-5 items-center gap-2 text-xs ${copyState === "error" ? "text-amber-200" : "text-emerald-200"}`}
        ...
        <button
          className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded bg-emerald-400 px-4 text-sm font-bold text-emerald-950 transition-colors hover:bg-emerald-300 disabled:cursor-wait disabled:opacity-70"
