...
use crate::models::{
    ...
    RuntimeAssignmentDelivery, RuntimeObservation,
};
...
impl RuntimeApi {
    ...
    pub async fn assignments_update(
        &self,
        node_uuid: &str,
        environment: &str,
        body: &PublishRuntimeAssignmentRequest,
        idempotency_key: &str,
    ) -> Result<RuntimeAssignment, SdkworkError> {
        let path = custom_path(&format!(
            "/web/runtime_assignments/{}/{}",
            serialize_path_parameter(
                node_uuid,
                PathParameterSpec::new("nodeUuid", "simple", false)
            ),
            serialize_path_parameter(
                environment,
                PathParameterSpec::new("environment", "simple", false)
            )
        ));
    ...
    pub async fn assignments_current_retrieve(
        &self,
        environment: &str,
        if_generation: Option<&str>,
        if_snapshot_sha256: Option<&str>,
    ) -> Result<RuntimeAssignmentDelivery, SdkworkError> {
        ...
        let path = append_query_string(
            custom_path(&"/web/runtime_assignments/current".to_string()),
            &query,
        );
    ...
    pub async fn assignments_observations_create(
        &self,
        snapshot_uuid: &str,
        body: &CreateRuntimeObservationRequest,
        idempotency_key: &str,
    ) -> Result<RuntimeObservation, SdkworkError> {
        let path = custom_path(&format!(
            "/web/runtime_assignments/{}/observations",
            serialize_path_parameter(
                snapshot_uuid,
                PathParameterSpec::new("snapshotUuid", "simple", false)
            )
        ));
        let headers = build_request_headers(
            &[(
    ...
    pub async fn assignments_observations_latest_retrieve(
        &self,
        snapshot_uuid: &str,
    ) -> Result<RuntimeObservation, SdkworkError> {
        let path = custom_path(&format!(
            "/web/runtime_assignments/{}/observations/latest",
            serialize_path_parameter(
                snapshot_uuid,
                PathParameterSpec::new("snapshotUuid", "simple", false)
            )
