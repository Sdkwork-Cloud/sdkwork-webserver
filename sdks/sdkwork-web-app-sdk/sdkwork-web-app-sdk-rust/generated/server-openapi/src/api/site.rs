...
impl SiteApi {
    ...
    /// 创建站点
    pub async fn sites_create(&self, body: &CreateSiteRequest, idempotency_key: &str) -> Result<SiteResponse, SdkworkError> {
        let path = app_path(&"/sites".to_string());
    ...
    /// 获取站点详情
    pub async fn sites_retrieve(&self, site_id: &str) -> Result<SiteResponse, SdkworkError> {
    ...
    /// 删除站点
    pub async fn sites_delete(&self, site_id: &str) -> Result<(), SdkworkError> {
        let path = app_path(&format!("/sites/{}", serialize_path_parameter(site_id, PathParameterSpec::new("siteId", "simple", false))));
        self.client.delete(&path, None, None).await
    }

    /// 激活站点
    pub async fn sites_activate(&self, site_id: &str) -> Result<SiteResponse, SdkworkError> {
        let path = app_path(&format!("/sites/{}/activate", serialize_path_parameter(site_id, PathParameterSpec::new("siteId", "simple", false))));
        self.client.post(&path, Option::<&serde_json::Value>::None, None, None, None).await
    }

    /// 暂停站点
    pub async fn sites_pause(&self, site_id: &str) -> Result<SiteResponse, SdkworkError> {
        let path = app_path(&format!("/sites/{}/pause", serialize_path_parameter(site_id, PathParameterSpec::new("siteId", "simple", false))));
        self.client.post(&path, Option::<&serde_json::Value>::None, None, None, None).await
    }
