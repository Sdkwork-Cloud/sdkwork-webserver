...
class SiteApi {
  ...
  /// 创建站点
  Future<SitesCreateResponse201?> sitesCreate(CreateSiteRequest body, String idempotencyKey) async {
    ...
    final response = await _client.post(ApiPaths.appPath('/sites'), body: payload, headers: requestHeaders, contentType: 'application/json');
    return (() {
      final map = sdkworkResponseAsMap(response);
  ...
  /// 删除站点
  Future<void> sitesDelete(String siteId) async {
    await _client.delete(ApiPaths.appPath('/sites/${serializePathParameter(siteId, const PathParameterSpec('siteId', 'simple', false))}'));
  }
