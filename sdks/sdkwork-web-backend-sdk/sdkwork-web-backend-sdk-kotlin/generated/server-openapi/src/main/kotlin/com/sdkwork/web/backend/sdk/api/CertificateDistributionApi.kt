...
class CertificateDistributionApi(private val client: HttpClient) {

    /** List certificate manifest convergence by server */
    suspend fun certificatesDistributionList(page: Int? = null, pageSize: Int? = null): CertificatesDistributionListResponse? {
        val query = buildQueryString(listOf(
            QueryParameterSpec("page", page, "form", true, false, null),
            QueryParameterSpec("page_size", pageSize, "form", true, false, null)
        ))
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.backendPath("/certificate_distribution"), query))
        return client.convertValue(raw, object : TypeReference<CertificatesDistributionListResponse>() {})
    }
