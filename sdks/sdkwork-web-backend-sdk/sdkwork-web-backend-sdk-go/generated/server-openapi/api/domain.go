...
func (a *DomainApi) DomainsList(page *int, pageSize *int) (sdktypes.DomainsListResponse, error) {
    ...
    raw, err := a.client.Get(AppendQueryString(BackendApiPath("/domains"), query), nil, nil)
    if err != nil {
...
func (a *DomainApi) DomainsCreate(body sdktypes.CreateManagedDomainRequest, idempotencyKey string) (sdktypes.DomainsCreateResponse201, error) {
    headers := BuildRequestHeaders(
        map[string]ParameterSpec{"Idempotency-Key": ParameterSpec{Value: idempotencyKey, Style: "simple", Explode: false},},
        map[string]ParameterSpec{},
    )
    raw, err := a.client.Post(BackendApiPath("/domains"), body, nil, headers, "application/json")
    if err != nil {
        var zero sdktypes.DomainsCreateResponse201
        return zero, err
    }
    return decodeResult[sdktypes.DomainsCreateResponse201](raw)
}

// Delete an unbound tenant custom domain asset
func (a *DomainApi) DomainsDelete(domainId string, idempotencyKey string) (struct{}, error) {
    headers := BuildRequestHeaders(
        map[string]ParameterSpec{"Idempotency-Key": ParameterSpec{Value: idempotencyKey, Style: "simple", Explode: false},},
        map[string]ParameterSpec{},
    )
    raw, err := a.client.Delete(BackendApiPath(fmt.Sprintf("/domains/%s", SerializePathParameter(domainId, PathParameterSpec{Name: "domainId", Style: "simple", Explode: false}))), nil, headers)
    if err != nil {
        var zero struct{}
        return zero, err
    }
...
func (a *DomainApi) DomainsVerify(domainId string, idempotencyKey string) (sdktypes.DomainsVerifyResponse, error) {
    ...
    raw, err := a.client.Post(BackendApiPath(fmt.Sprintf("/domains/%s/verify", SerializePathParameter(domainId, PathParameterSpec{Name: "domainId", Style: "simple", Explode: false}))), nil, nil, headers, "")
    if err != nil {
        var zero sdktypes.DomainsVerifyResponse
        return zero, err
    }
    return decodeResult[sdktypes.DomainsVerifyResponse](raw)
}

// Bind a tenant custom domain to an application
func (a *DomainApi) DomainsApplicationBindingUpdate(domainId string, body sdktypes.UpdateDomainApplicationBindingRequest, idempotencyKey string) (sdktypes.DomainsApplicationBindingUpdateResponse, error) {
    headers := BuildRequestHeaders(
        map[string]ParameterSpec{"Idempotency-Key": ParameterSpec{Value: idempotencyKey, Style: "simple", Explode: false},},
        map[string]ParameterSpec{},
    )
    raw, err := a.client.Put(BackendApiPath(fmt.Sprintf("/domains/%s/application_binding", SerializePathParameter(domainId, PathParameterSpec{Name: "domainId", Style: "simple", Explode: false}))), body, nil, headers, "application/json")
    if err != nil {
        var zero sdktypes.DomainsApplicationBindingUpdateResponse
        return zero, err
    }
    return decodeResult[sdktypes.DomainsApplicationBindingUpdateResponse](raw)
}

// Unbind a tenant custom domain from its application
func (a *DomainApi) DomainsApplicationBindingDelete(domainId string, idempotencyKey string) (struct{}, error) {
    headers := BuildRequestHeaders(
        map[string]ParameterSpec{"Idempotency-Key": ParameterSpec{Value: idempotencyKey, Style: "simple", Explode: false},},
        map[string]ParameterSpec{},
    )
    raw, err := a.client.Delete(BackendApiPath(fmt.Sprintf("/domains/%s/application_binding", SerializePathParameter(domainId, PathParameterSpec{Name: "domainId", Style: "simple", Explode: false}))), nil, headers)
    if err != nil {
        var zero struct{}
        return zero, err
    }
