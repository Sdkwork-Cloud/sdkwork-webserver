...
type ApplicationSourceVersionApi struct {
    client *sdkhttp.Client
}

func NewApplicationSourceVersionApi(client *sdkhttp.Client) *ApplicationSourceVersionApi {
    return &ApplicationSourceVersionApi{client: client}
}
...
func (a *ApplicationSourceVersionApi) ApplicationsSourceVersionsCreate(applicationId string, body sdktypes.CreateApplicationSourceVersionRequest, idempotencyKey string) (sdktypes.ApplicationsSourceVersionsCreateResponse201, error) {
    headers := BuildRequestHeaders(
        map[string]ParameterSpec{"Idempotency-Key": ParameterSpec{Value: idempotencyKey, Style: "simple", Explode: false},},
        map[string]ParameterSpec{},
    )
    raw, err := a.client.Post(BackendApiPath(fmt.Sprintf("/applications/%s/source_versions", SerializePathParameter(applicationId, PathParameterSpec{Name: "applicationId", Style: "simple", Explode: false}))), body, nil, headers, "application/json")
    if err != nil {
...
func (a *ApplicationSourceVersionApi) ApplicationsSourceVersionsRetrieve(applicationId string, sourceVersionId string) (sdktypes.ApplicationsSourceVersionsRetrieveResponse, error) {
    raw, err := a.client.Get(BackendApiPath(fmt.Sprintf("/applications/%s/source_versions/%s", SerializePathParameter(applicationId, PathParameterSpec{Name: "applicationId", Style: "simple", Explode: false}), SerializePathParameter(sourceVersionId, PathParameterSpec{Name: "sourceVersionId", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
