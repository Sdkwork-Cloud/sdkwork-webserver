...
class CertificateApi {
  ...
  Future<CertificatesCreateResponse201?> certificatesCreate(CreateCertificateRequest body, String idempotencyKey) async {
    ...
    final response = await _client.post(ApiPaths.backendPath('/certificates'), body: payload, headers: requestHeaders, contentType: 'application/json');
    return (() {
      final map = sdkworkResponseAsMap(response);
  ...
  Future<CertificatesRenewResponse?> certificatesRenew(String certificateId, String idempotencyKey) async {
    final requestHeaders = buildRequestHeaders(
      <String, HeaderParameterSpec>{
        'Idempotency-Key': HeaderParameterSpec(idempotencyKey, 'simple', false, null),
      },
    ...
    return (() {
      final map = sdkworkResponseAsMap(response);
      return map == null ? null : CertificatesRenewResponse.fromJson(map);
    })();
  }
}
