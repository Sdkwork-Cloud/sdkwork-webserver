...
export interface DomainApplicationBindingDeleteParams {
  idempotencyKey: string;
}

export class DomainApplicationBindingApi {
...
/** List tenant custom domain assets */
  async list(params?: DomainListParams, requestOptions?: ApiRequestOptions): Promise<{ items: ApplicationDomainResponse[]; pageInfo: PageInfo; }> {
    const query = buildQueryString([
      { name: 'page', value: params?.page, style: 'form', explode: true, allowReserved: false },
      { name: 'page_size', value: params?.pageSize, style: 'form', explode: true, allowReserved: false },
    ]);
...
/** Register a tenant custom domain asset */
  async create(body: CreateManagedDomainRequest, params: DomainCreateParams, requestOptions?: ApiRequestOptions): Promise<ApplicationDomainResponse> {
    const requestHeaders = buildRequestHeaders(
      {
        'Idempotency-Key': { value: params.idempotencyKey, style: 'simple', explode: false },
      },
