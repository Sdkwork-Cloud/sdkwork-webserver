...
use crate::models::{ApplicationSourceVersionResponse, CreateApplicationSourceVersionRequest, ImportApplicationGitSourceVersionRequest};

#[derive(Clone)]
pub struct ApplicationSourceVersionApi {
    client: Arc<SdkworkHttpClient>,
}

impl ApplicationSourceVersionApi {
    pub fn new(client: Arc<SdkworkHttpClient>) -> Self {
        Self { client }
    }

    /// List immutable application source versions
    pub async fn applications_source_versions_list(&self, application_id: &str, page: Option<i64>, page_size: Option<i64>) -> Result<serde_json::Value, SdkworkError> {
        let query = build_query_string(&[
            QueryParameterSpec::new("page", page, "form", true, false, None),
            QueryParameterSpec::new("page_size", page_size, "form", true, false, None),
        ]);
        let path = append_query_string(backend_path(&format!("/applications/{}/source_versions", serialize_path_parameter(application_id, PathParameterSpec::new("applicationId", "simple", false)))), &query);
        self.client.get(&path, None, None).await
    }

    /// Register an immutable Drive-backed application source version
    pub async fn applications_source_versions_create(&self, application_id: &str, body: &CreateApplicationSourceVersionRequest, idempotency_key: &str) -> Result<ApplicationSourceVersionResponse, SdkworkError> {
        let path = backend_path(&format!("/applications/{}/source_versions", serialize_path_parameter(application_id, PathParameterSpec::new("applicationId", "simple", false))));
    ...
    pub async fn applications_source_versions_retrieve(&self, application_id: &str, source_version_id: &str) -> Result<ApplicationSourceVersionResponse, SdkworkError> {
        let path = backend_path(&format!("/applications/{}/source_versions/{}", serialize_path_parameter(application_id, PathParameterSpec::new("applicationId", "simple", false)), serialize_path_parameter(source_version_id, PathParameterSpec::new("sourceVersionId", "simple", false))));
        self.client.get(&path, None, None).await
    }
