...
#[derive(Clone)]
pub struct CertificateDistributionApi {
    client: Arc<SdkworkHttpClient>,
}

impl CertificateDistributionApi {
    pub fn new(client: Arc<SdkworkHttpClient>) -> Self {
        Self { client }
    }

    /// List certificate manifest convergence by server
    pub async fn certificates_distribution_list(
        &self,
        page: Option<i64>,
        ...
        let path = append_query_string(
            backend_path(&"/certificate_distribution".to_string()),
            &query,
        );
