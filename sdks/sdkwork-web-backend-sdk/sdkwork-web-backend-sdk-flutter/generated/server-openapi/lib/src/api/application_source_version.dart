...
class ApplicationSourceVersionApi {
  ...
  Future<ApplicationsSourceVersionsCreateResponse201?> applicationsSourceVersionsCreate(String applicationId, CreateApplicationSourceVersionRequest body, String idempotencyKey) async {
    ...
    final response = await _client.post(ApiPaths.backendPath('/applications/${serializePathParameter(applicationId, const PathParameterSpec('applicationId', 'simple', false))}/source_versions'), body: payload, headers: requestHeaders, contentType: 'application/json');
    return (() {
      final map = sdkworkResponseAsMap(response);
  ...
  Future<ApplicationsSourceVersionsRetrieveResponse?> applicationsSourceVersionsRetrieve(String applicationId, String sourceVersionId) async {
    final response = await _client.get(ApiPaths.backendPath('/applications/${serializePathParameter(applicationId, const PathParameterSpec('applicationId', 'simple', false))}/source_versions/${serializePathParameter(sourceVersionId, const PathParameterSpec('sourceVersionId', 'simple', false))}'));
    return (() {
      final map = sdkworkResponseAsMap(response);
