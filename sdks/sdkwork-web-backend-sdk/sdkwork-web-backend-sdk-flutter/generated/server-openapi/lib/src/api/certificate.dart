...
class CertificateApi {
  ...
  Future<CertificatesIssueResponse202?> certificatesIssue(IssueCertificateRequest body, String idempotencyKey) async {
    ...
    final payload = body.toJson();
    final response = await _client.post(ApiPaths.backendPath('/certificates/issue'), body: payload, headers: requestHeaders, contentType: 'application/json');
    return (() {
      final map = sdkworkResponseAsMap(response);
  ...
  Future<CertificatesOperationsRetrieveResponse?> certificatesOperationsRetrieve(String operationId) async {
    final response = await _client.get(ApiPaths.backendPath('/certificates/operations/${serializePathParameter(operationId, const PathParameterSpec('operationId', 'simple', false))}'));
    return (() {
      final map = sdkworkResponseAsMap(response);
  ...
  Future<CertificatesUpdateResponse?> certificatesUpdate(String certificateId, UpdateCertificateRequest body, String idempotencyKey) async {
    final requestHeaders = buildRequestHeaders(
      <String, HeaderParameterSpec>{
        'Idempotency-Key': HeaderParameterSpec(idempotencyKey, 'simple', false, null),
      },
    ...
    final payload = body.toJson();
    final response = await _client.put(ApiPaths.backendPath('/certificates/${serializePathParameter(certificateId, const PathParameterSpec('certificateId', 'simple', false))}'), body: payload, headers: requestHeaders, contentType: 'application/json');
    return (() {
      final map = sdkworkResponseAsMap(response);
  ...
  Future<CertificatesRenewResponse202?> certificatesRenew(String certificateId, String idempotencyKey) async {
    final requestHeaders = buildRequestHeaders(
      <String, HeaderParameterSpec>{
        'Idempotency-Key': HeaderParameterSpec(idempotencyKey, 'simple', false, null),
      },
    ...
    final response = await _client.post(ApiPaths.backendPath('/certificates/${serializePathParameter(certificateId, const PathParameterSpec('certificateId', 'simple', false))}/renew'), headers: requestHeaders);
    return (() {
      final map = sdkworkResponseAsMap(response);
